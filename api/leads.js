...
function getCorsHeaders(req) {
  ...
  if (!origin) return {};
  try {
    if (new URL(origin).host === req.headers.host || allowedOrigins.includes(origin)) {
      return {
...
function isRateLimited(ipAddress) {
  ...
  const now = Date.now();
  const bucket = submissionBuckets.get(key) || { count: 0, resetAt: now + rateLimitWindowMs };
...
export default async function handler(req, res) {
  ...
  const ipAddress = getClientIp(req.headers, req.socket?.remoteAddress);
  if (isRateLimited(ipAddress)) return res.status(429).json({ error: "Too many requests." });
  ...
  try {
    const { data: savedLead, error } = await saveLead({ supabase, lead, userAgent: req.headers["user-agent"], ipAddress });
    if (error) {
      console.error("Failed to insert demo lead", error);
